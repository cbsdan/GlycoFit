...
class FoodBaselineAssessment:
        ...
        for key, value in responses.items():
            ...
            if question_type == 'scale':
                ...
                if value in options:
                    ...
                    # For protective factors (negative weight), reverse the score
                    if risk_weight < 0:
                    ...
                    total_score += score
                    max_possible_score += abs(risk_weight) * 100
                ...
                if key == 'daily_meal_frequency':
                    ...
                    optimal = 3.5
                    deviation = abs(numeric_value - optimal)
                    score = min(deviation * risk_weight * 25, abs(risk_weight) * 100)
                    
                elif key in ['vegetable_servings', 'fruit_servings']:
                    # More is better (protective factors)
                    # Optimal: 5+ servings vegetables, 2-3 fruits
                    optimal = 5 if key == 'vegetable_servings' else 2.5
                    if numeric_value >= optimal:
                        score = abs(risk_weight) * 100  # Maximum protective benefit
                    else:
                        score = (numeric_value / optimal) * abs(risk_weight) * 100
                        
                elif key == 'snacking_frequency':
                    # More snacks = higher risk
                    score = min(numeric_value * risk_weight * 25, abs(risk_weight) * 100)
                    
                elif key == 'water_intake':
                    # 8 glasses is optimal
                    optimal = 8
                    if numeric_value >= optimal:
                        score = abs(risk_weight) * 100
                    else:
                        score = (numeric_value / optimal) * abs(risk_weight) * 100
    ...
    def update_baseline_risk_score(user_id):
        """Recalculate and update baseline risk score"""
        try:
            db = get_db()
            ...
            responses = baseline.get('responses', {})
            risk_score = FoodBaselineAssessment.calculate_baseline_risk_score(responses)
            
            # Update the baseline with new risk score
            result = db.food_baseline_assessments.update_one(
                {'user_id': ObjectId(user_id) if isinstance(user_id, str) else user_id},
                {
                    '$set': {
            ...
            log_database_operation('update_one', 'food_baseline_assessments', {'user_id': user_id}, result)
